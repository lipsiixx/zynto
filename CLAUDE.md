...
Есть обёртка `z.ps1` (`.\z.ps1 <команда>`) поверх сырых команд ниже — использует Tailscale IP сервера и ключ из `.ssh`, читает `POSTGRES_PASSWORD` из `.env`:

| Команда | Действие |
|---------|----------|
| `bot` | `python main.py` — запуск бота с API (polling) |
| `ui` | `npm run dev` в `miniapp/` → dev-сервер на :5173 |
| `build-ui` | сборка мини-аппа → `static/miniapp/` (нужно перед деплоем) |
...
Тестов (unit/pytest) в проекте нет — проверка изменений через `compileall` и ручной запуск polling.
...
- Деплой продакшена — через Docker (`docker-compose.yml`): контейнеры `db` (postgres:16), `redis` (redis:7), `bot`. `DATABASE_URL`/`REDIS_URL` внутри compose переопределяются на имена сервисов, не берутся из `.env`.
...
| Клавиатуры | `keyboards/` | `user_kb.py`, `admin_kb.py` — InlineKeyboardMarkup-фабрики. |
| Утилиты | `utils/` | `formatters.py` (форматирование дат/текста), `pagination.py`, `code_generator.py` (генерация промокодов). |
| Разовые скрипты | `scripts/` | Ручной запуск вне бота, например `purge_media.py`. |
...
По просьбе автора (см. README): спорь, если предложение пользователя неоптимально — не соглашайся молча, аргументируй альтернативу.
